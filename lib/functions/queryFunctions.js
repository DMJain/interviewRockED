const createQueryForSearch = (query) => {
    ...
    const {search = null,status = null,filterDate = null ,name = null, orderDate = null, limit = null, page = 1} = query
    ...
    if(search) {
        if(status || filterDate){seacrhQuery = seacrhQuery + ` AND name '%${search}%'`}
        else {
            seacrhQuery = seacrhQuery + ` WHERE name LIKE '%${search}%'`
        }
    }
